	...
	<table class="table">
	  <tbody>
	    ...
	    <tr>
	      ...
	      <td><a href="{{ route('posts.show', ['post' => $statistics['longest_post']->slug]) }}">{{ $statistics['longest_post']->title }}</a> | {{ $statistics['longest_post']->text_length }}</td>
	    <tr>
	      <th scope="row">Самая короткая статья</th>
	      <td><a href="{{ route('posts.show', ['post' => $statistics['shortest_post']->slug]) }}">{{ $statistics['shortest_post']->title }}</a> | {{ $statistics['shortest_post']->text_length }}</td>
	    <tr>
